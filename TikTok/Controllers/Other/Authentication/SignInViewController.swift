    ...
    @objc private func didTapSignIn() {
        ...
        guard let email = emailField.text,
              ...
              password.count >= 6 else {
                  ...
                  let alert = UIAlertController(title: "Woops", message: "Please enter a valid email and password to sigh in.", preferredStyle: .alert)
        ...
        AuthManager.shared.signIn(with: email, password: password) { [weak self] result in
            DispatchQueue.main.async {
                ...
                case .failure(let error):
                    print(error)
                    let alert = UIAlertController(title: "Sign In Failed", message: "Please check your email and password and try again.", preferredStyle: .alert)
                    alert.addAction(UIAlertAction(title: "Dismiss", style: .cancel))
